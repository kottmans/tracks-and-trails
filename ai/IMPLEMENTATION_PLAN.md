...
follow-ups at `53b07ec`**, so criterion 6 is no longer waiting on a deliverable. Criterion 8, added by maintainer ruling on 2026-08-04, is **met on the maintainer's evidence**: the 40-row built-window run passed on `kirk`, 2026-08-05 (`ai/evidence/2026-08-05-criterion-8-third-run.md`), and CI is green on the candidate. **Criterion 6 is the only live gate.** Criterion 6 was requested on 2026-08-05 and is in progress. `ai/REVIEWS.md` holds every verdict, and **no tally is kept here** — a count beside the record is a second copy of it, and it drifts the moment another verdict lands. It read *"three times"* against five. *(This said "criterion 6 is the only thing left" and was written before criterion 8 existed — a sentence that stayed true-looking while the criteria changed underneath it.)*
