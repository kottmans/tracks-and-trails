...
Three items, all tracked and none open-ended:

| # | Owed | Why it is not a phase problem |
|---|---|---|
| 0 | **Criterion 8's own findings** — nine open from the 2026-08-04 review, two of them ruled the same day (`T140-R3`, `T140-R5`) | Criterion 8, now **Not met**. `T-140` is reopened for group verbs, count-bearing removal and a keyboard-reachable disclosure; a playlist's format moves to the group. Exact-head Windows evidence is owed after the corrections, not before — the review says to spend that run on a candidate rather than on this one. *(The paste-of-150 measurement `UX-005` said could reopen the shape did come back clear — 0.002 s to open, 0.019 s to measure, 0.022 s to paint against a 0.5 s budget — and the reviewer upheld the instrument. The shape stands; its implementation does not yet.)* |
