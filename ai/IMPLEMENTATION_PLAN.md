...
|---|---|---|
| 0 | **Criterion 8 — the checklist has been run, and it found eleven things** | **Run on 2026-08-05 at `f2ec6b7`**, per `docs/CRITERION_8_CHECKLIST.md`; the record is `ai/evidence/2026-08-05-criterion-8-checklist-run.md`. Every finding of the 2026-08-04 round and its three correction passes was already resolved and approved at `431bb47`. **The run then produced `T-149`–`T-159` — none reported by any gate, against a green suite of 2153 tests.** That measures `P2EXIT-R10`'s argument rather than arguing it. **Seven belong to the closed list** — `T-149` to `T-132`; `T-151` and `T-152` to `T-134`/`T-135`; `T-153` to `T-137`; `T-154`, `T-155` and `T-157` to `T-140` — and four are new scope. **The classification is a maintainer ruling and is what criterion 8 now waits on**, together with rows 3.6 and §5, which were not run: the first needed a failure the checklist did not explain how to arrange, the second a theme control that does not exist (`T-146`). |
