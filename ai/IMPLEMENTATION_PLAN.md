...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the nine tasks found by running the application on 2026-08-04 | **Met 2026-08-04.** `T-132`–`T-141` are all complete and committed; `T-132`–`T-139` and `T-141` are green on all five CI jobs including `windows desktop`, and `T-140` is owed the same run. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |
...
|---|---|---|
| 0 | **Windows evidence for `T-140`** — the only push since the last green run | Criterion 8. Every other task in it already has it. *(This row held `T-140`'s model half until 2026-08-04, when it landed. **The measurement `UX-005` said could reopen the shape came back clear**: 150 entries across ten open playlists cost 0.002 s to open, 0.019 s to measure and 0.022 s to paint a viewport, against a 0.5 s budget. The shape stands.)* |
