...
| 6 | Reviewed and signed off | **Not met, now for two separately-tracked reasons.** (a) The **phase** sign-off is still absent — every deliverable is reviewed, and the exit review itself requested changes on 2026-08-04 for `T-122` and the `UX-005` batch; those corrections are submitted and await re-review. (b) **`T-128` must diagnose the result-pump segfault first** — maintainer ruling of 2026-08-04 on `P2EXIT-R9`, recorded as an amendment to `OPS-007`, whose zero-recurrence premise `T-128` removed. *(This row read "**Not met — but not for the reason this row gave until 2026-08-03**" and named only (a).)* | This row read *"Four deliverables await review: `T-100`, `T-086`, `T-084`, `T-082`"* for two days after all four were approved — `T-082` at `b1b7cd6`, `T-084` and `T-086` at `2a41c5f`, `T-100` at `c242dd3`, all 2026-08-01. What criterion 6 still wants is what Phase 0 and Phase 1 each needed: an independent exit review over this table, by a reviewer that is not the implementer (`AGENTS.md` §3). Phase 1's found two wrong rows (`P1EXIT-R1`, `P1EXIT-R2`), so the table is expected to be rebuilt against the repository before it is submitted, not merely re-read |
