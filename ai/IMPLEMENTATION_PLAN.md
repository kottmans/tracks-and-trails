...
| 7 | *(Found by `T-088`)* A user can actually start a queue | **NOT MET — `T-115`**, confirmed High by review | Measured: five URLs, limit 3 — three run and complete, **two stay `queued` with an empty pool**. Nothing admits durable `QUEUED` rows; `start()` raises when full instead of parking; the dialog starts only the probed job. `test_every_queued_job_eventually_starts_as_slots_free` asserts the **wanted** outcome under `xfail(strict=True)`, so closing `T-115` turns it `XPASS` and reddens the build until it is inverted (`T088-R1`: the previous unconditional `pytest.xfail()` could never do that) |
