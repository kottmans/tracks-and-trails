...
|---|---|---|
| 0 | **Criterion 8 — seven findings from its own checklist** | **Run on 2026-08-05 at `f2ec6b7`**; record at `ai/evidence/2026-08-05-criterion-8-checklist-run.md`. It produced `T-149`–`T-159`, **none reported by any gate**, against a green suite of 2153 tests — which measures `P2EXIT-R10`'s argument rather than arguing it. **Ruled 2026-08-05:** seven are inside criterion 8 — `T-149`, `T-151`, `T-152`, `T-153`, `T-154`, `T-155`, `T-157` — and four are Phase 3: `T-150`, `T-156`, `T-158`, `T-159`. **The line is what the criterion asserts**: a defect where accepted work is *unreachable or drawn wrong* contradicts *the window matches the features behind it*; a defect asking for something *new* does not. That is `T140-R5`'s own reasoning, and it keeps the closed list closed — no task is added to `T-132`–`T-141`; these are findings against them. **Also owed:** checklist rows 3.6 and §5, never run — the first needed a failure the checklist did not explain how to arrange, the second a theme control that does not exist (`T-146`). Then the checklist re-runs on the candidate. |
