...
|---|---|---|
| 0 | **Criterion 8's own findings** — the 2026-08-04 round and the two correction rounds that followed | Criterion 8 is **Not met** until a re-review says otherwise. `T140-R3`, `T132-R2` and `COORD-R22` are **resolved**; `T140-R6`, `T137-R2`, `T137-R3` and `T140-R5` are **resolved at `083fbe4`**. `T-140`'s reopened criteria are **built** — group verbs, count-bearing removal and a keyboard-reachable disclosure — with **`Pause all` deferred to `REQ-017`** by the 2026-08-05 amendment to `UX-005`, because holding one group has no mechanism once `T-080` deleted `JobStatus.PAUSED`. What is owed is the re-review itself and then exact-head Windows and Fedora evidence on the candidate, which the review says to spend on a candidate rather than on a head already known to be wrong. |
