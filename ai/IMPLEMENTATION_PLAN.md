...
| An unsupported URL produces a failed job showing the extractor's own message | **Still open — `P1EXIT-R2`.** `test_a_failure_reaches_the_parent_classified_and_verbatim` does exercise a real `run_session`, queue carriage, protocol validation and exact message equality — but it raises `GeoRestrictedError` and asserts `GEO_RESTRICTED`. It never constructs `UnsupportedError` or observes `UNSUPPORTED_URL`, so **it is not evidence for this row**; the correction repeated the original mapping defect with a different unrelated error class. `test_yt_dlps_bug_report_boilerplate_does_not_reach_the_user` uses a generic `ExtractorError` and has the same problem. No live unsupported URL is sent through yt-dlp either |
| Worker code runs with no display attached | **Not met — `P1EXIT-R1` is still open.** The *environment* half is now real: `test_the_worker_really_runs_with_no_display_attached` spawns an interpreter with `DISPLAY` and `WAYLAND_DISPLAY` removed and asserts inside the child that they are gone, mutation-verified. But the *workload* half is split away from it — that child only calls `_import_ytdlp()`, never `run_session`, a protocol sequence, or any worker work. The test that does those things, `test_the_worker_runs_in_a_real_spawned_process_with_no_display`, **inherits the desktop environment** while its name and docstring call the child headless. A display dependency introduced anywhere after yt-dlp resolution would leave this criterion green |
