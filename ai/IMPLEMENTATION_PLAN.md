...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the **ten** tasks found by running the application on 2026-08-04, `T-132`–`T-141` | **Not met.** *Called met on 2026-08-04 and reset the same day by `P2EXIT-R10`.* The claim was made while this row's own evidence column said `T-140`'s exact-head Windows run was owed, and the review then found defects the unit gates had not. *(This cell also cited `T132-R1`, a primary action said not to be filled in the real toolbar. **That finding was withdrawn as reviewer error on 2026-08-05** — the enabled product route is brand-filled, and the neutral button observed was `UX-005` row 6 declining to fill a *disabled* one. The claim is removed rather than softened, because it was evidence for this row's verdict and it was wrong.)* `T-140` was reopened for the accepted criteria it did not build (`T140-R5`) and they are now built, `Pause all` excepted — deferred to `REQ-017` by the 2026-08-05 amendment to `UX-005`. Of the round's findings, `T140-R3`, `T132-R2` and `COORD-R22` are resolved and the rest are corrected and awaiting re-review; `T140-R6` was a Critical found in the correction itself, a group offering retry for DRM against `SEC-001`. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |
...
|---|---|---|
| 0 | **Criterion 8's own findings, and the evidence they name** | Criterion 8 is **Not met** until a re-review says otherwise. `T140-R3`, `T132-R2` and `COORD-R22` are **resolved**; the implementation findings `T140-R6`, `T137-R2`, `T137-R3` and `T140-R5` are **resolved at `083fbe4`**; the record correction is at `c920446` and awaits its own resolution. `T-140`'s reopened criteria are **built** — group verbs, count-bearing removal and a keyboard-reachable disclosure — with **`Pause all` deferred to `REQ-017`** by the 2026-08-05 amendment to `UX-005`. **Three things are owed, and the first is not automation.** (a) The built application run against a **written checklist derived from `T-132`–`T-141` and the adopted mockups, on the exact candidate head** — `P2EXIT-R10` requires it because five visible defects escaped the gates and `T132-R1` was visible in the real window immediately, so **automated checks alone are not sufficient evidence for a criterion about what the window looks like**, and Windows/Fedora automation does not replace it. The checklist is evidence for the closed list, not an invitation to add tasks. (b) Exact-head Windows and Fedora evidence on that same candidate, which the review says to spend on a candidate rather than on a head already known to be wrong. (c) The re-review itself. |
...
| 2 | **The independent Phase 2 exit review** over this table | `AGENTS.md` §3. Phase 0 and Phase 1 each needed one, and Phase 1's found two wrong rows — so the table is rebuilt against the repository before submission rather than re-read. |
