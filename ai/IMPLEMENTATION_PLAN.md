...
|---|---|---|
| 0 | **Criterion 8's evidence** — its findings are closed | **Every finding of the 2026-08-04 round and its three correction passes is resolved**, `P2EXIT-R10` included, approved at `431bb47`. `T-140` is **Complete**: the three accepted criteria it was reopened for are built, with **`Pause all` deferred to `REQ-017`** by the 2026-08-05 amendment to `UX-005`. **The criterion is still Not met, because what it now waits on is evidence rather than work.** (a) The built application run against a **written checklist derived from `T-132`–`T-141` and the adopted mockups, on the exact candidate head** — `P2EXIT-R10` requires it because five visible defects escaped the gates and `T132-R1` was visible in the real window immediately, so **automated checks alone are not sufficient evidence for a criterion about what the window looks like**, and Windows/Fedora automation does not replace it. The checklist is evidence for the closed list, not an invitation to add tasks. (b) Exact-head Windows and Fedora evidence on that same candidate. |
