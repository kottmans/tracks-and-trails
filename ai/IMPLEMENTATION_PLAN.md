...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the nine tasks found by running the application on 2026-08-04 | **Not met.** *Called met on 2026-08-04 and reset the same day by `P2EXIT-R10`.* The claim was made while this row's own evidence column said `T-140`'s exact-head Windows run was owed, and the review then found visible defects in the built window — including a primary action that **is not filled in the real toolbar** (`T132-R1`), which was reported as working on the strength of a test that styled a stand-in. `T-140` is reopened for the accepted criteria it did not build (`T140-R5`). Nine reviewer findings are open. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |
...
unfalsifiable — nine tasks came out of one afternoon at the window, and a tenth sitting would find
a tenth task. The criterion therefore names **`T-132` through `T-141`** by id and nothing else.
...
| 1 | **A clean 60-run Linux soak** of the full suite against the corrected teardown (`T-128`, `OPS-007`) | The crash it measures was in the **test harness** — a fixture dropping a `QObject` that still owned a running `QTimer`. Nothing in `src/` reads `is_idle`, and the application never drops a manager. Sixty runs is sized against the measured 2-in-39 baseline: unchanged, a clean sixty has probability 0.042. |
