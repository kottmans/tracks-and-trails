...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the nine tasks found by running the application on 2026-08-04 | **Eight of nine met.** `T-132`–`T-139` and `T-141` are complete, committed and green on all five CI jobs including `windows desktop`. **`T-140` is outstanding**: the group row is drawn and nothing answers its roles, so a playlist still shows as unrelated rows. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |
...
|---|---|---|
| 0 | **`T-140`'s model half** — `QueueModel` flattening playlist groups into rows, expansion held by id, the count and segments derived from members, and `setUniformItemSizes` off with the paste-of-150 measurement its own criteria demand | Criterion 8, ruled 2026-08-04. The drawing is committed at `bc1a3d4` and nothing drives it. **`UX-005` says a bad measurement justifies reopening the shape**, so this is the one outstanding item that could still change a decision rather than merely take time |
