...
follow-ups at `53b07ec`**, so criterion 6 is no longer waiting on a deliverable. **It is not the only thing left**: criterion 8 was added by maintainer ruling on 2026-08-04 and is Not met, awaiting the 40-row built-window run. Criterion 6 was requested on 2026-08-05 and is in progress. `ai/REVIEWS.md` holds every verdict, and **no tally is kept here** — a count beside the record is a second copy of it, and it drifts the moment another verdict lands. It read *"three times"* against five. *(This said "criterion 6 is the only thing left" and was written before criterion 8 existed — a sentence that stayed true-looking while the criteria changed underneath it.)*
...
| 6 | Reviewed and signed off | **Not met — and what remains is now one thing, not two.** *(b), the clean 60-run soak, was met on 2026-08-05: 60 of 60 with no process deaths at `ef21e34`, P = 0.042 against the 2-in-39 baseline. What is left is (a), the independent phase exit review. **It was requested on 2026-08-05** — `ai/handoffs/2026-08-05-phase-2-exit-review.md` — and is **in progress**. `ai/REVIEWS.md` holds every verdict, and **no tally is kept here** — a count beside the record is a second copy of it, and it drifts the moment another verdict lands. It read *"three times"* against five. This row said *"has never been requested"* for a day after it was, which is `P2EXIT-R14` exactly.* The original wording follows.** (a) The **phase** sign-off is absent. Every deliverable is reviewed; the 2026-08-04 exit review requested changes for `T-122` and the `UX-005` batch, those corrections were made, re-reviewed and **approved**, and are committed at `05ad990` with all five CI jobs green including `windows desktop`. What is owed is the independent review *of this table*. (b) **A clean 60-run Linux soak** against the corrected teardown — `OPS-007`, ruled 2026-08-04. *(This row named `T-128`'s diagnosis as the blocker; `T-128` is diagnosed, found the harness rather than the product, and the maintainer replaced the prerequisite with the measurement below. Before that it read "**Not met — but not for the reason this row gave until 2026-08-03**" and named only (a).)* | This row read *"Four deliverables await review: `T-100`, `T-086`, `T-084`, `T-082`"* for two days after all four were approved — `T-082` at `b1b7cd6`, `T-084` and `T-086` at `2a41c5f`, `T-100` at `c242dd3`, all 2026-08-01. What criterion 6 still wants is what Phase 0 and Phase 1 each needed: an independent exit review over this table, by a reviewer that is not the implementer (`AGENTS.md` §3). Phase 1's found two wrong rows (`P1EXIT-R1`, `P1EXIT-R2`), so the table is expected to be rebuilt against the repository before it is submitted, not merely re-read |
...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the **ten** tasks found by running the application on 2026-08-04, `T-132`–`T-141` | **Not met.** *Claimed met on 2026-08-05 and reset by `P2EXIT-R12` the same day — the second time this row has been claimed over the top of its own evidence, after `P2EXIT-R10`.* **Two checklist runs.** The first (`ai/evidence/2026-08-05-criterion-8-checklist-run.md`) found **eleven** defects against 2153 green tests, none reported by any gate; seven were ruled inside this criterion and are Complete. The second (`ai/evidence/2026-08-05-criterion-8-second-run.md`) attempted all 41 rows on `kirk`, Fedora, and **39 passed**: **row 2.7 failed** (`T-160`, the format control over the thumbnail) and **row 3.15 failed** (`T-161`, every playlist drawing a blank parent picture). Both are since dispositioned — see *what is owed* below — and that historical count is not recomputed. It was first recorded as *"pass, all 41 rows"*, corrected in that file. **Filing a failure as a Phase 3 task does not make the row a pass** — the closed-list rule decides which task owns a defect, not what a row observed. **Row 3.15 is accepted Phase 2 work**: `T-153`'s criterion is that the staged playlist row *shows* its picture, and its regression proves only that an address is selected. `T-161` is that task's unfinished half and is reclassified Phase 2. **What is owed:** a re-run over the remaining **40** rows, recording passed rows separately from failed ones. `T-161` is corrected, and row 2.7 was removed by `T161-R1` — it had been authored after the closed list to describe `T-160`, and keeping it made a Phase 3 task a Phase 2 gate. Its property moved to `T-160` unweakened. **Rows 3.6 and §5 have now been run** — 3.6 caught a completed playlist drawing blank blocks (`T-140`'s colour fix reading `palette.highlight()` on a widget whose palette carries the selection tint by `T130-R1`'s design), fixed at `6bae7ec`. *(This cell also cited `T132-R1`, withdrawn as reviewer error on 2026-08-05.)* `T-140` was reopened by `T140-R5` and is **Complete**, `Pause all` excepted and deferred to `REQ-017`. `T140-R6` remains the one worth remembering: a Critical found in the *correction*, where a group offered retry for DRM against `SEC-001`. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |

### What stands between here and the exit, as of 2026-08-04
...
| 2 | **The independent Phase 2 exit review** over this table — **requested 2026-08-05, in progress** | `AGENTS.md` §3. Phase 0 and Phase 1 each needed one, and Phase 1's found two wrong rows — so the table is rebuilt against the repository before submission rather than re-read. It has requested changes across `P2EXIT-R11` (criterion 1, a stale stage), `R12` (a false checklist pass), `R13` (focus theft), `R14` (records disagreeing with each other) and `T161-R1` — all resolved except `R12`, which awaits the 40-row run. `ai/REVIEWS.md` holds every verdict, and **no tally is kept here** — a count beside the record is a second copy of it, and it drifts the moment another verdict lands. It read *"three times"* against five. `R11`, `R13` and `T161-R1` are resolved. |
