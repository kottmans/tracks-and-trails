...
| 8 | *(Added 2026-08-04 by maintainer ruling)* **The window matches the features behind it** — the **ten** tasks found by running the application on 2026-08-04, `T-132`–`T-141` | **Claimed met, 2026-08-05 — the maintainer's verdict, offered to the exit review rather than asserted past it.** *This row was called met on 2026-08-04 and reset the same day by `P2EXIT-R10`, for claiming a verdict its own evidence column contradicted. It is stated differently now: the evidence is named, and so is what the evidence does not reach.* **Two checklist runs.** The first (`ai/evidence/2026-08-05-criterion-8-checklist-run.md`) found **eleven** defects against 2153 green tests, none reported by any gate; seven were ruled inside this criterion and fixed. The second (`ai/evidence/2026-08-05-criterion-8-second-run.md`) passed **all 41 rows** on `kirk`, Fedora, across `6bae7ec..541b484` — a range rather than a sha, because the maintainer did not record which was checked out and `git diff --stat` across it is `ai/TASKS.md` alone. **Rows 3.6 and §5 ran for the first time**, and 3.6 failed immediately: a completed playlist drew blank blocks, because `T-140`'s colour fix read `palette.highlight()` on a widget whose palette carries the quiet selection tint by `T130-R1`'s design — `#ebf1ee` where the application answers `#1e5e47`. Fixed at `6bae7ec`. **The limits are part of the claim:** one platform — nobody has *looked* at the window on Windows, though CI runs the suite there — one runner, who is also the person who accepted the mockups, and four known defects present during the run (`T-161`–`T-164`, Phase 3, listed in the checklist so they are not re-filed). *(This cell also cited `T132-R1`, withdrawn as reviewer error on 2026-08-05; the claim is removed rather than softened, because it was evidence for a verdict and it was wrong.)* `T-140` was reopened by `T140-R5`, is now **Complete** — `Pause all` excepted, deferred to `REQ-017` — and every finding of that round and its three correction passes is resolved, `P2EXIT-R10` included, approved at `431bb47`. `T140-R6` remains the one worth remembering: a Critical found in the *correction*, where a group offered retry for DRM against `SEC-001` because a second surface for an existing action was built without carrying its guard across. | Each task carries its own regressions and killed mutations; the heads are `7311180`, `d16ccfe`, `bc1a3d4`, `66750f8`, `5a6a91f`, `60a6943`, `6c70db3`. **`T-137` is why this is a criterion rather than a preference**: the queue read *Playlist (16 items)* and downloaded one file, so the phase named "Queue and concurrency" would have exited on a queue that misreported its own contents |
