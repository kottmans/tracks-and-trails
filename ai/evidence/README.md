...
| `2026-08-20-linux-orphans-on-kirk.md` | **Read-only state of two processes that no longer exist, which is the rule working rather than a caveat on it.** PIDs `432922` and `434366` were orphaned on `kirk` from 2026-08-16; the file captures `/proc`, `ps` and `eu-stack` output taken while they ran. **Both ended within hours of the capture** — `2026-08-20T06:20:48Z`, no reboot, nothing signalled — so the subject is gone and nothing regenerates it: the parent was already gone, so even a fresh orphan would be a different specimen. They were **preserved rather than reaped** (`T258-R4`, `T-272`) and expired anyway; this file is the whole of what survives. |
