...
`collect_submodules` stays, as insurance against a pin we do not have yet. What remains is
implementation: **extend the frozen probe** so it loads the built-in solver through yt-dlp's real
`vendor.load_script` path and verifies its hash — after which removing `collect_data_files` must
*fail*, which today it does not.
*(This read "Blocked on `T033-R4` — a maintainer decision, not a machine", 2026-07-29;
re-triaged 2026-08-03.)* The external Windows build half is available: `frozen windows` runs on
`STARBASE` and passes. What remains is the probe extension and the separate-submodule decision,
which no runner supplies. The Linux
...
**Phase:** Phase 3
**Depends on:** `T-140` (done)
**Relevant context:** `UX-005` row 9 and the mockup's verb table, `DAT-005` §4, `ui/queue_view.py`,
`ui/row_verbs.py`, `ui/main_window.py`
**Affected surfaces:** `ui/row_verbs.py`, `ui/queue_view.py`, `ui/main_window.py`
**Risk:** Medium — every one of them acts on many jobs at once

#### Scope

`T-140` draws the header and gives it a count, a bar and a disclosure. It gives it **no verbs**, so
a playlist can only be acted on one entry at a time. The mockup's table names four:
...
rather than a bare *Remove*.

#### Acceptance criteria

- Each verb acts on **every member**, asserted with a group whose members are in different states
- `Retry failed` is absent when nothing failed, and present when something did
- `Cancel all` and `Remove` each name their count in the confirmation (`DAT-005` §4)
...
*Filed retrospectively on 2026-08-05, and renumbered from `T-146` on merge:* the maintainer had
already published a `T-146` — the Settings screen — while this was local, so two different tasks
briefly carried one id. That is the second thing `COORD-R22`'s boundary hid: an id claimed in
source before it was claimed in `TASKS.md` cannot be checked against anyone else's.

*Filed retrospectively.* The work was written and committed while the code cited a
