...
--
-- v7 added `history.request` (`T-159`), so a completed download can be named in the words the
-- rest of the window uses instead of yt-dlp's format id. Every state a later migration would
-- have to preserve is seeded:
--
--   * `h7-converted` -- a record carrying its request, which is what v7 exists for. The stored
...
--     request. The worse half of the same defect.
--   * `h7-legacy`    -- the nulls a row written before v7 leaves behind: an id and no request.
--     A migration could lose the request column's absence without this, and those rows are every
--     history row in a database that predates v7.
--
...
CREATE TABLE history (
    ...
    format_used  TEXT,
    bytes_total  INTEGER,
    completed_at TEXT NOT NULL, thumbnail_url TEXT, playlist_id TEXT, playlist_index INTEGER, playlist_title TEXT, request TEXT,

    CHECK (bytes_total IS NULL OR bytes_total >= 0)
);
INSERT INTO "history" VALUES('h7-converted','https://example.invalid/v7-one','Track one','/downloads/Trail Sounds/Track one.mp3','251',5242880,'2026-08-05T21:00:00+00:00','https://img.invalid/one.jpg','pl-7',0,'Trail Sounds','{"audio_codec": "mp3", "audio_quality": "192", "cookies_from_browser": null, "embed_subtitles": false, "format_selector": "bestaudio/best", "media_kind": "audio", "output_directory": "/downloads/Trail Sounds", "output_template": "%(title)s.%(ext)s", "post_processors": [], "proxy": null, "rate_limit_bytes": null, "subtitle_languages": [], "url": "https://example.invalid/v7-one"}');
INSERT INTO "history" VALUES('h7-merged','https://example.invalid/v7-two','A merged download','/downloads/A merged download.mp4','399+140',15728640,'2026-08-05T21:01:00+00:00',NULL,NULL,NULL,NULL,'{"audio_codec": "best", "audio_quality": null, "cookies_from_browser": null, "embed_subtitles": false, "format_selector": "bestaudio/best", "media_kind": "video", "output_directory": "/downloads/Trail Sounds", "output_template": "%(title)s.%(ext)s", "post_processors": [], "proxy": null, "rate_limit_bytes": null, "subtitle_languages": [], "url": "https://example.invalid/v7-era"}');
