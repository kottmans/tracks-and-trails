...
def test_a_child_stopped_in_the_window_dies_with_no_outer_job_to_reap_it(tmp_path: Path) -> None:
    ...
    **What this costs `T-258` is stated in its entry, not here**: the outer Job is defence in
    ...
    try:
        ...
        parent.kill()
        parent.wait(timeout=30)
        # **This test expects a survivor on Windows, so cleanup is not optional here.** Leaving it
        # would make the suite the thing that accumulates the orphans the task is about.
        if child is not None:
            with contextlib.suppress(psutil.NoSuchProcess):
                child.kill()
