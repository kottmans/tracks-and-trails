...
| 2.5 | **Shift+F10** opens the full menu at both widths. **Press it before touching anything at all** — not merely before clicking a *row*. `T-152` failed twice for two different reasons: first the list had no current row, then the key was going to the toolbar's concurrency stepper instead of the list (that stepper has since left the toolbar with `UX-013`, so a failure here now means the key went nowhere at all). Only an untouched window asks both questions at once. Then again after clicking a row | `T-135`, `T-152`, `NFR-005` |
