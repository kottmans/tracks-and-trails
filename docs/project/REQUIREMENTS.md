...
- **NFR-002 — Startup (Linux).** Cold start to interactive window under 3 seconds on the reference Linux machine. **Measured 2026-09-12 on the AppImage: 0.634 s warm, 1.077 s first launch after a build** (`T-325`). *(No cold sample is taken for `0.1.0`, by the maintainer's ruling — `REL-008`'s 2026-09-13 amendment.)* *(The words "reference Linux machine" are load-bearing and were read past once: this requirement has never covered Windows, and `TESTING.md` §8 item 14 said "the reference machine" without them, which is how a Windows figure came to be reported against it. Windows has its own number in `NFR-010`.)*
...
- **NFR-007 — Privacy.** No telemetry, no analytics, no phone-home, no crash reporting to a third party. The only outbound network traffic is downloads the user requested, explicit yt-dlp update checks, **SponsorBlock lookups when the user has switched them on** (`SEC-003`, 2026-08-07), **and a check for a newer Tracks & Trails release: once a day unless switched off, and whenever the user asks** (`REL-009`, 2026-09-13). *(The third destination was added deliberately rather than the promise read loosely. It is opt-in per preset, and the request carries the first four hex characters of `sha256(video_id)` — one bucket in 65,536 — with filtering done locally, so the endpoint does not learn which video was watched. Measured against yt-dlp 2026.07.04, not recalled. A configurable API endpoint was declined: a typo in it would be a new destination.)* *(The fourth destination, GitHub's API for this repository's latest release, was added the same way on 2026-09-13 by the maintainer's ruling: one `GET` carrying no identifier, no settings and not even the running version, because the comparison is made locally. It is **on by default**, which the SponsorBlock lookups are not, and that difference was the ruling's point: a user who never hears of a release keeps a broken build.)*
