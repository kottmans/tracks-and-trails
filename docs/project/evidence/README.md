...
| Windows | `tools/windows/sandbox_evidence.sh docs/project/evidence/windows-<version>.md <installer>`, in Windows Sandbox on `STARBASE`; exits non-zero unless the report says PASS |

**Both are scripts now.** *(This said the Windows half was by hand and covered *cancel another*;
`T-318` scripted it through Sandbox's logon command, and `T318-R2` found the wording left behind.)*
Neither covers §8 item 8's *cancel another* or a normal exit — the Windows run force-stops the
application, and a download probe has no parent to cancel it — so that clause stays a sitting.
