...
deliberately rather than committed and its record is kept below. Ruled by the maintainer from the
built window: *"there is
...
#### Built 2026-09-10, second attempt

The first attempt's record is kept below because its warnings were right and its blocker was not
what it thought.

**Both blockers were the same shape: code that read as dead teardown was load-bearing.**

1. **The reparent had to be deferred, not just the page switch.** A panel is built with the staging
   list as its parent, so adding it to the page's layout *moves it out of the list* — and doing
   that inside `commitData` on the row's combo box is `T108-R2`'s defect through a different call.
2. **`setIndexWidget(index, None)` is what reaps the row's editor**, and nothing else does.
   `QAbstractItemView::indexWidget` returns the *editor* registered for an index, so setting it to
   `None` removes and deletes it. Hiding the list never closes it, and Qt's `shouldEdit` then
   refuses every later `edit()` on that index. `RowDelegate.commit_and_close_editor` is **not** a
   substitute — it clears the delegate's reference, not the view's registration — and was tried
   twice. Both call sites now say what breaks if they are removed.

**What found them was bisecting, not reasoning.** An isolated Qt reproduction proved the mechanism
innocent — neither a page switch nor a model reset while hidden breaks `edit()` — and rebuilding
one step at a time against the single failing test located each cause in minutes. Four guesses
preceded the second one and all four were wrong.

**Two product defects surfaced during the test split**, both cases where Qt had been doing
lifecycle work for free:

- `_on_list_resized` went on re-placing the panel into the row's rectangle, collapsing it from
  900x700 to 856x80 on **every refresh**. `relayout_panel` and `remount_panel` went with it.
- A panel outlived the row it belonged to when its URL line was deleted: Qt used to destroy the
  index widget with the row. `_refresh_once` now closes a panel whose row has gone.

**One over-deletion, corrected.** `_widen_for` was removed on the reasoning that a page is given
the dialog's size — but the *dialog* is still 666px wide, so the lists scrolled sideways. It is
back, and the page did remove its complexity: the chrome it used to measure changed when acted on,
which is why `T310-R2` needed a convergence loop. One measurement now.

**The tests split three ways**, as the first attempt predicted: **six retired** with the mechanism
(`T-296`, `T204-R4` twice, `T210-R1`, the open row being the only one that grows, and `T310-R2`'s
widening), replaced by one asserting what outlives them; **eight re-pointed** at the page; and two
fixed as real findings — `T-150`'s narrowing test compared a `QStackedWidget` against the button
box, and a container is always at least as wide as what it holds. The same defect was in
`tools/dialog_width_floor_probe.py`, which had started naming wrappers instead of the widget
refusing to shrink. The measured floor is **314px**, slightly below the 318px before.

**The tooltip filed with this task needed no fix, and that is asserted rather than claimed.** The
open row answered `ToolTipRole` with its collapsed summary, which appeared over the table being
read. A tooltip is presented by the view under the pointer, and that view is no longer on screen —
so the symptom goes with the mechanism. `test_an_open_panel_fills_the_page_and_keeps_its_way_out`
now asserts both halves together: the staging list is not visible, **and** the row still describes
itself. Split apart, either could pass while the defect returned.
...
- The panel fills the dialog. `panel_height_for` and `_mount_panel` are gone, along with the tests
  that exist only to guard their arithmetic — **and the guards that outlive them** (that no list
  scrolls sideways, that every row is reachable) are re-expressed against the page. *Corrected at
  the build:* this criterion named `_widen_for` too and was **wrong** — a page fills the dialog but
  does not widen it, and the dialog opens at 666px against a panel wanting ~1100. It stays, minus
  the convergence loop its removed chrome required.
