...
| 5.6 | **Watch the panel at the exact moment it opens — both kinds** (the format table via `Choose specific formats…`, and a playlist's entry panel): it arrives at full size, with nothing small flashing first. *(Was a maintainer-directed row carrying `T-221`'s second observation. **The direction was superseded on 2026-09-10**: both kinds used to spend one event-loop turn at 190×26 while being mounted **into the row**, and `T-312` replaced that mounting with a page swap, so what it asked to have confirmed cannot occur. Kept as an ordinary row — a page swap can land badly too — and it is no longer evidence anybody is owed.)* | `T-221`, `T-209`, `T-312` |
